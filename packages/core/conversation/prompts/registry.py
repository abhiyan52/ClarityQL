"""Prompt registry for intent classification version management."""

from .base import BaseIntentPrompt


class IntentPromptRegistry:
    """
    Registry for intent classification prompt versions.

    Allows registering and retrieving prompt versions by name.
    Supports "latest" as an alias for the most recent version.
    """

    _prompts: dict[str, type[BaseIntentPrompt]] = {}

    @classmethod
    def register(cls, prompt_class: type[BaseIntentPrompt]) -> type[BaseIntentPrompt]:
        """
    ...
    @classmethod
    def get(cls, version: str = "latest") -> BaseIntentPrompt:
        """
        ...
        """
        if not cls._prompts:
            raise ValueError("No intent prompts registered")
        ...
        if version not in cls._prompts:
            ...
            raise ValueError(
                f"Unknown intent prompt version: {version}. Available: {available}"
            )
    ...
    def list_versions(cls) -> list[str]:
        """List all registered intent prompt versions."""
